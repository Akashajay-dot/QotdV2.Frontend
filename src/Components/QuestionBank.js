...
Qid();
 ...
 const deleteQuestion2 = async ()=>{
  setPopover(false)
  try {
        const response = await axios.delete(`${apiBaseUrl}/api/delete/${QidtoDelte}`, {
          headers: {
              Authorization: `Bearer ${token}`,
          },
      });
...
<button className="searchbtn" onClick={dltsearch}>
 {(searchtxt!="") ?<CloseIcon/>:<SearchIcon/>}
      ...
      {popover && <div className="DeletePopOver">
        <HighlightOffIcon
         sx={{
      ...
      <h3>Are you sure?</h3>
      <p className="DeletePopOver-p">Do you really want to delete these records? <br />This process cannot be undone.

      </p>
    <div className="DeletePopOver-Btns">
    <button className="DeletePopOver-Btns-cancel DeletePopOver-Btn" onClick={()=>{setPopover(false)}}>Cancel</button>
    <button className="DeletePopOver-Btns-delete DeletePopOver-Btn" onClick={deleteQuestion2}>Delete</button>
    </div>
